...
class ChangeHadithPage extends StatefulWidget {
  const ChangeHadithPage({super.key, required this.hadithModel});
...
class _ChangeHadithPageState extends State<ChangeHadithPage> {
  ...
  @override
  void initState() {
    _hadithNumberController =
        TextEditingController(text: widget.hadithModel.hadithNumber);
    _hadithTitleController =
        TextEditingController(text: widget.hadithModel.hadithTitle);
    _hadithArabicController =
        TextEditingController(text: widget.hadithModel.hadithArabic);
    _hadithTranslationController =
        TextEditingController(text: widget.hadithModel.hadithTranslation);
    super.initState();
  }
          ...
          children: [
            ...
            OutlinedButton(
              onPressed: () {
                ...
                bool checkIsEmpty = _hadithNumberController.text.isNotEmpty &&
                                    _hadithTitleController.text.isNotEmpty &&
                                    _hadithArabicController.text.isNotEmpty &&
                                    _hadithTranslationController.text.isNotEmpty;

                bool checkIsEquality = _hadithNumberController.text != widget.hadithModel.hadithNumber ||
                                     _hadithTitleController.text != widget.hadithModel.hadithTitle ||
                                     _hadithArabicController.text != widget.hadithModel.hadithArabic ||
                                     _hadithTranslationController.text != widget.hadithModel.hadithTranslation;
                if (checkIsEmpty && checkIsEquality) {
                  Provider.of<HadithDataState>(context, listen: false)
                      .changeHadith(hadithId: widget.hadithModel.id, mapHadith: mapHadith);
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        duration: const Duration(seconds: 1),
                        backgroundColor: appColors.secondary,
                        content: const Text(AppString.changed,
  ...
  bool checkIsEmpty() {
    return _hadithNumberController.text.isNotEmpty &&
        _hadithTitleController.text.isNotEmpty &&
        _hadithArabicController.text.isNotEmpty &&
        _hadithTranslationController.text.isNotEmpty;
  }
